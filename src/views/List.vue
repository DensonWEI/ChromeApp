<template>
  <v-app class="denson_list">
    <div>
      ...
      <!-- 图片内容区 -->
      <v-container id="scroll-target" class="scroll-y" style="height: calc(100vh - 56px)" grid-list-lg ref="father"
        @click="getDetil">
        <v-layout v-scroll:#scroll-target="onScroll" row wrap ref="child">
          <v-flex v-for="(n, i) in imgList" :key="i" xs6 d-flex v-ripple class="myimgbox">
            <img :class="['elevation-4', 'myImg']" :src="n.thumbUrl" alt="" />
    ...
    <!-- 回到顶部 -->
    <v-fab-transition>
      <v-btn v-show="!hidden" fixed dark fab bottom right :loading="load" ref="button" @click="topTop" color="pink">
        <v-icon>keyboard_arrow_up</v-icon>
      </v-btn>
    </v-fab-transition>
    <!-- 提示 -->
    <v-snackbar v-model="snackbar" :color="snackbarColor" :timeout="6000">{{ snackbarTxt }}
...
if (process.env.NODE_ENV === 'development') {
  ...
  api = 'https://pic.sogou.com/napi/wap/pic'
}
export default {
  data: () => ({
    distance: 30,
  ...
  methods: {
    ...
    onScroll(e) {
      ...
      if (
        childH - (e.target.scrollTop + this.$refs.father.offsetHeight - 32) <=
        this.distance &&
        !this.load
      ) {
        console.log('触底，需要加载')
    ...
    doSearch() {
      ...
      fetch(
        `${api}?query=${this.value} 表情&start=${this.page * 48 + 1}&xml_len=48`
      )
        .then((data) => {
          data.json().then((res) => {
            if (res.data.items) {
